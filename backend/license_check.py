...
from app_paths import resolve_data_path, resolve_resource, runtime_root
...
def _resolve_path(rel_or_abs: str) -> Path:
    return resolve_data_path(rel_or_abs)
...
def check_license(
    ...
    *,
    ...
    if path.is_absolute():
        ...
        lic_path = resolve_resource(license_file)

    try:
        expires, _msg = verify_license_file(lic_path, clock=clock, today=today)
